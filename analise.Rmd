...
author: "author"
abstract: "A maioria das análises utilizando esse banco de dados remove
...
knitr::opts_chunk$set(echo=F,message=F,warning=F,fig.pos = 'H',fig.align = 'center',fig.width=6, fig.height=3.7)
...
A análise de sobrevivência difere dos métodos estatísticos clássicos pois temos dados censuradas, que são observações  a qual o evento de interesse por algum motivo não pode ser obtido. 
...
As crônicas de gelo e fogo possui uma quantidade enorme de personagens, assim fica claro que o número de personagens que morre é muito menor que os que não morrem, portanto foi removido da análise todos os personagens que não estavam associados a alguma família relevante ^[Onde relevante é definido arbitrariamente pelo  autor que vos escreve].
...
```{r, fig.width=6.7,fig.cap="Comparação entre quantidade de membros nas associações e mortos"}
...
death |> 
  ggplot(aes(Allegiances, total, fill = Allegiances)) + 
  geom_bar(stat = 'identity') + 
  coord_flip() +
  theme(legend.position = "none") +
death |> 
  ggplot(aes(Allegiances, prop, fill = Allegiances)) + 
  geom_bar(stat = 'identity') + 
  coord_flip() +
  theme(legend.position = "none")
...
Como é de se esperar, personagens que são de uma família nobre tem uma sobrevivência estimada superior que os que são de família comum.
...
As analises anteriores são pertinentes para a seleção das covariáveis do modelo
de regressão, pois como vimos existem evidências de diferença nas funções de 
sobrevivência nesse grupos, ou seja dependendendo do grupo a qual o personagem
...
Assim temos como p-valor associado ao teste 0.067, 0.879  para as variáveis Genêro, Nobreza, respectivamente. Portanto indicando que não existem indícios
