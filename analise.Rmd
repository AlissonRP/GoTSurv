...
author: "Alisson Rosa"
abstract: "A maioria das análises ^[Podem ser consultadas [aqui](https://www.kaggle.com/datasets/mylesoneill/game-of-thrones/code?datasetId=43&sortBy=voteCount)] empregando o banco de dados  aqui utilizado remove
...
o que foi desenvolvido aqui portanto é uma análise de sobrevivência onde foi 
avaliado
...
dada a definição de $\Lambda$ em , e sabendo que a função acumulada de uma 
distribuição exponencial com parâmetro igual a 1 é dada por $\exp(-x)$, dessa 
maneira temos:
...
Podemos notar em ambasa as figuras que visualmente não existem evidências de
...
### Comandos para obten��o dos gr�ficos de sobreviv�ncia
b <- fit$coefficients
###### Sobreviv�ncia estimada para a idade=50
...
###### Gr�fico das curvas de sobreviv�ncia  para a idade=50
par(mfrow = c(1, 2), cex = 0.8)
...
### Para o gr�fico da taxa de falha acumulada estimada
## Taxas de falha
Ht1 <- -log(st1)
Ht2 <- -log(st2)
...
discrepância torna-se maior ainda quando coloca-se personagens femininos nobres.
