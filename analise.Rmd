...
Sabemos que a proporção de personagens do sexo feminino é de apenas aproximadamente $16$%, assim uma pergunta a se fazer é se existe diferença na sobrevivência entre os sexos, o que pode ser 
avaliado pela figura NN
...
```{r, fig.cap="Função de sobrevivência estimada  para as classes sociais"}
...
```{r, fig.cap="Função de sobrevivência estimada  para as classes sociais e sexos"}
...
sobrevivência para eles e também com algumas famílias, o que pode ser visto pela figura NN


```{r, fig.cap="Função de sobrevivência estimada para as Associações 1"}
df |> 
filter(Allegiances %in% c("House Stark", "House Targaryen",
...
Assim a figura NN nos indica que a vida de um Stark não é fácil, a cada livro lançado até o livro 3, a sobrevivência para um Stark despenca, e no livro 3 ^[Red wedding para os mais familiarizados] podemos ver a maior queda. 
Podemos notar também que os Baratheon iniciam com uma sobrevivência superior ao Lannister, mas um comportamento que não se mantém a partir do livro 2.

Podemos também avaliar a sobrevivência para o povo livre, a patrulha da noite e a junção das famílias que agora são denotadas por "outros", o que pode ser visto pela figura


```{r, fig.cap="Função de sobrevivência estimada para as Associações 2"}
df |> 
filter(Allegiances %in% c("Wildling", "Night's Watch"
                          , "Outros")) |> 
survfit(Surv(time_death, status) ~ Allegiances,
  data = _
) |>
...
Notamos que até o livro 2 temos um comportamento semelhante, porém o Povo livre e a Patrulha da noite tomam um grande abate no livro 3 ^[Exploração além das muralhas], e como era de se esperar, no livro 5 o grupo dos "outros" formado somente por nobres mantém a maior sobrevivência.
...
O p-valor do teste para cada grupo é indicado pela tabela TT
```{r}
...
test_family_1 <- survdiff(Surv(time_death, status) ~ Nobility, rho = 0, 
                          data = df |> 
filter(Allegiances %in% c("House Stark", "House Targaryen",
                          "House Lannister", "House Baratheon")))

test_family_2 <- survdiff(Surv(time_death, status) ~ Nobility, rho = 0, 
                          data = df |> 
filter(Allegiances %in% c("Wildling", "Night's Watch"
                          , "Outros")))
...
```
Assim podemos averiguar que o único grupo a qual o teste não captou diferença
...
Podemos também naturalmente comparar a o efeito da interação entre essas duas 
covariáveis na taxa de falha, supondo a título de curiosidade um personagem feminino e nobre, 
